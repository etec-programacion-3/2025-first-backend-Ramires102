...
export const AutorProducto = async (req, res) => {
    try {
        console.log("Buscando libros por autor: ", req.query.Autor);
        ...
        const libro = await Libros.findAll({
            where: {
                Autor: req.query.Autor // Asume que tu modelo tiene un campo 'autor'
                }});
        if (!libro) {
...
export const TituloProducto = async (req, res) => {
    try {
        ...
        const libro = await Libros.findAll({
            where: {
                titulo: req.query.Titulo // Asume que tu modelo tiene un campo 'titutlo'
            }});
        if (!libro) {
            return res.status(404).json({ error: 'Libros no se an encontrado segun el autor' });
        }
